...
class Downsize(nn.Module):
    ...
    def __init__(self):
        ...
        self.stem2 = conv_bn_relu(
            16, 3, kernel_size=1, stride=1, padding=0)
...
class PeleeNet(nn.Module):
    ...
    def __init__(self, phase, size, cfg=None):
        ...
        for i, num_layers in enumerate(cfg.block_config):
            ...
            if i != len(cfg.block_config) - 1:
                ...
                #        i + 1), nn.AvgPool2d(kernel_size=2, stride=(2,4), ceil_mode=False))
                ...
                '''
                       if i != len(cfg.block_config) - 1:
                    if i+1 == 2:
                    #elf.features.add_module('transition%d_pool' % (
                    #   i + 1), nn.AvgPool2d(kernel_size=2, stride=(2,4), ceil_mode=False))
                    #um_features = num_features
                    #lse:
                    self.features.add_module('transition%d_pool' % (
                        i + 1), nn.AvgPool2d(kernel_size=2, stride=2, ceil_mode=True))
                    num_features = num_features'''
        ...
        nchannels = [256, 512, 704, 256, 256, 256]
    ...
    def forward(self, x):
        ...
        for k, feat in enumerate(self.features):
            #if self.features.transition2_pool
            #if k == 6:
            #    x = torch.nn.functional.pad(x,(2,2,0,0), value=0)
            x = feat(x)
            #print(x.shape,k)
