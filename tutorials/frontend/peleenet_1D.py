...
class conv_bn_relu(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, activation=True, kernel_size=3, stride=1, padding=0):
        super(conv_bn_relu, self).__init__()
        #self.conv = nn.Conv2d(in_channels, out_channels, bias=False, **kwargs)
        #self.norm = nn.BatchNorm2d(out_channels)
        #self.activation = activation
        ...
        if kernel_size==1:
            ...
            self.conv1dw = nn.Conv2d(in_channel, out_channel, kernel_size=1, bias=False)
            self.bn1 = nn.BatchNorm2d(out_channel)
            
            # Horizontal convolution definition
            self.conv1h = nn.Conv2d(out_channel, out_channel, kernel_size=(1,kernel_size), stride=(1,stride), padding=(0,padding), groups=out_channel, bias=False)
            self.bn2 = nn.BatchNorm2d(out_channel)
            
            # Vertical convolution definition 
            self.conv1v = nn.Conv2d(out_channel, out_channel, kernel_size=(kernel_size,1), stride=(stride,1), padding=(padding,0), groups=out_channel, bias=False)
