...
def calibrate_dataset_peleenet(img_path):

	_preprocess = BaseTransform(304, (103.94, 116.78, 123.68), (2, 0, 1))
	im_fnames = sorted((fname for fname in os.listdir(img_path) 
		if os.path.splitext(fname)[-1] == '.jpg'))
	im_fnames = (os.path.join(img_path, fname) for fname in im_fnames)
...
def quantize(mod, params,img_path, data_aware,b_size=1):
	if data_aware:
		with relay.quantize.qconfig(calibrate_mode='kl_divergence', weight_scale='max'):
			mod = relay.quantize.quantize(mod, params, dataset=calibrate_dataset_peleenet(img_path))
	else:
